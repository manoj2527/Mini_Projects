...
void produce(void)
{
	...
	//   ...
	//   ...
	acquire_mutex_spinlock(2);
	while(get_mutex_value(2)>=MAX_LIMIT){
		cond_wait(3,2);
	}
	set_mutex_value(2,get_mutex_value(2)+1);
	cond_signal(2);
	printf(1, "[%d]: Produced an item. Curernt buffer size: %d\n", getpid(), get_mutex_value(2));
	release_mutex_spinlock(2);
}
